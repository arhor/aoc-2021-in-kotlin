...
public class Day14 {
    ...
    public static void main(String[] args) {
        String path = "src/main/resources/day14.txt";
    ...
    private static Map<String, Long> transform(Map<String, Long> record) {
        var newRecord = new HashMap<String, Long>();

        for (String key : record.keySet()) {
            Long countKey = record.get(key);

            if (insertions.containsKey(key)) {
                final var insertion = insertions.get(key);

                String insert1 = insertion[0];
                String insert2 = insertion[1];
